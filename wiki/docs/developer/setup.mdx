...
If a `<ModAsset>` (or item asset) image matches an existing ingame item, the actual ingame item (or blockitem) will be displayed instead. This means that you don't need to include the rendered textures of your items.
All images are expected in the ".assets/modid/" folder, not the ".assets/item/modid" subfolder. Content wikis will have their item properties collected
from the ingame registries, instead of using the predefined json files.
