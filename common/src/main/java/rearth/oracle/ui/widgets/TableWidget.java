...
public class TableWidget extends UIComponent {
    ...
    public record Cell(Text text, FlowWidget.HorizontalAlignment alignment) {
    }
    ...
    public TableWidget(List<List<Cell>> cells, boolean hasHeader, Predicate<String> linkHandler) {
        ...
        for (var row : cells) {
            List<LabelWidget> labels = new ArrayList<>(columns);
            for (int i = 0; i < columns; i++) {
                var cell = i < row.size() ? row.get(i) : new Cell(Text.empty(), FlowWidget.HorizontalAlignment.LEFT);
